...
Codex MUST NOT modify application code while `ACTIVE_TASK: NONE`.
...
Only one task may be marked active for Codex execution at a time.

When implementation is approved, change the machine-readable header to:
...
and include the exact task definition below before handing work to Codex.
